...
{
    /// <inheritdoc />
    public partial class ExternalChatIdToString : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // ponytail: dynamic SQL for steps referencing newly added columns — SQL Server compiles IF/BEGIN bodies at parse time
        ...
        {
            migrationBuilder.Sql("""
                IF TYPE_NAME(COLUMNPROPERTY(OBJECT_ID('MessageChannels'), 'ExternalChatId', 'SystemTypeId')) IN ('nvarchar', 'varchar')
                   AND COL_LENGTH('MessageChannels', 'ExternalChatId_old') IS NULL
                    ALTER TABLE MessageChannels ADD ExternalChatId_old bigint NOT NULL
                        CONSTRAINT DF_MessageChannels_ExternalChatId_old DEFAULT 0;
                """);

            migrationBuilder.Sql("""
                IF COL_LENGTH('MessageChannels', 'ExternalChatId_old') IS NOT NULL
                   AND COL_LENGTH('MessageChannels', 'ExternalChatId') IS NOT NULL
                    EXEC(N'UPDATE MessageChannels SET ExternalChatId_old = TRY_CAST(ExternalChatId AS bigint)');
                """);
            ...
            migrationBuilder.Sql("""
                IF COL_LENGTH('MessageChannels', 'ExternalChatId_old') IS NOT NULL
                   AND EXISTS (
                       SELECT 1 FROM sys.default_constraints
                       WHERE parent_object_id = OBJECT_ID('MessageChannels')
                         AND name = 'DF_MessageChannels_ExternalChatId_old')
                    ALTER TABLE MessageChannels DROP CONSTRAINT DF_MessageChannels_ExternalChatId_old;
                """);

            migrationBuilder.Sql("""
                IF COL_LENGTH('MessageChannels', 'ExternalChatId_old') IS NOT NULL
                   AND TYPE_NAME(COLUMNPROPERTY(OBJECT_ID('MessageChannels'), 'ExternalChatId', 'SystemTypeId')) IN ('nvarchar', 'varchar')
                    ALTER TABLE MessageChannels DROP COLUMN ExternalChatId;
                """);
            ...
            migrationBuilder.Sql(
                ...
                IF COL_LENGTH('MessageChannels', 'ExternalChatId_old') IS NOT NULL
                   AND COL_LENGTH('MessageChannels', 'ExternalChatId') IS NULL
                    EXEC(N'EXEC sp_rename ''MessageChannels.ExternalChatId_old'', ''ExternalChatId'', ''COLUMN''');
                """,
                suppressTransaction: true);

            migrationBuilder.Sql("""
                IF TYPE_NAME(COLUMNPROPERTY(OBJECT_ID('BaleMessages'), 'ChatId', 'SystemTypeId')) IN ('nvarchar', 'varchar')
                   AND COL_LENGTH('BaleMessages', 'ChatId_old') IS NULL
                    ALTER TABLE BaleMessages ADD ChatId_old bigint NOT NULL
                        CONSTRAINT DF_BaleMessages_ChatId_old DEFAULT 0;
                """);

            migrationBuilder.Sql("""
                IF COL_LENGTH('BaleMessages', 'ChatId_old') IS NOT NULL
                   AND COL_LENGTH('BaleMessages', 'ChatId') IS NOT NULL
                    EXEC(N'UPDATE BaleMessages SET ChatId_old = CASE WHEN ChatId = '''' THEN 0 ELSE TRY_CAST(ChatId AS bigint) END');
                """);
            ...
            migrationBuilder.Sql("""
                IF COL_LENGTH('BaleMessages', 'ChatId_old') IS NOT NULL
                   AND EXISTS (
                       SELECT 1 FROM sys.default_constraints
                       WHERE parent_object_id = OBJECT_ID('BaleMessages')
                         AND name = 'DF_BaleMessages_ChatId_old')
                    ALTER TABLE BaleMessages DROP CONSTRAINT DF_BaleMessages_ChatId_old;
                """);

            migrationBuilder.Sql("""
                IF COL_LENGTH('BaleMessages', 'ChatId_old') IS NOT NULL
                   AND TYPE_NAME(COLUMNPROPERTY(OBJECT_ID('BaleMessages'), 'ChatId', 'SystemTypeId')) IN ('nvarchar', 'varchar')
                    ALTER TABLE BaleMessages DROP COLUMN ChatId;
                """);
            ...
            migrationBuilder.Sql(
                ...
                IF COL_LENGTH('BaleMessages', 'ChatId_old') IS NOT NULL
                   AND COL_LENGTH('BaleMessages', 'ChatId') IS NULL
                    EXEC(N'EXEC sp_rename ''BaleMessages.ChatId_old'', ''ChatId'', ''COLUMN''');
                """,
                suppressTransaction: true);
        }
    }
