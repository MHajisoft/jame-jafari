...
- **خروج با بازگشت (PWA موبایل نصب‌شده):** در تب‌های اصلی، دکمه/ژست بازگشت سیستم پس از بار دوم (با پیام راهنما) اپ را می‌بندد؛ لایه اول: شیت/اورلی → فرم → صفحات تو در تو → سپس خروج. تب‌های پایین با `replace` جابه‌جا می‌شوند تا تاریخچه روی هم انباشته نشود.
